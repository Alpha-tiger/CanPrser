...
def parseJ1939(rawdata):
    ...
    with open("CANparsed.txt", "a") as text_file:
        ...
        #print("Source Address is {0}".format(int(SA,16)))

        # checks if the PGN parsed as int is available in the library
        if int(PGN,16) in J1939.keys():
            print("PGN {} Found in dictionary, associated values are {} ".format(int(PGN, 16), Values),file=text_file)
            if int(PGN,16) == 65226:
                if (b1 != 'FF'):
                    t_Hexbyte = int(b1, 16)
         ...
         if (int(PGN, 16)):
            print ("PGN {} Not found in dictionary, associated values are ".format(int(data[3:7],16)),Values,file=text_file)
    return;
